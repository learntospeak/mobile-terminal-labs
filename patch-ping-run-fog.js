(function(){
  ...
  let active=true,current='YOUR-PC',moving=false;
  ...
  function setCurrent(id){const c=center(id),w=q('#worm');if(!c||!w)return;current=id;qa('.node').forEach(n=>n.classList.toggle('current',n.dataset.id===id));w.style.left=(c.x-w.offsetWidth/2)+'px';w.style.top=(c.y-45)+'px';classify();renderCommandBuilder()}
  ...
  function animateTo(target){if(moving)return;const path=shortestPath(current,target);if(!path||path.length<2)return;moving=true;let i=1;const step=()=>{setCurrent(path[i]);i++;if(i<path.length)setTimeout(step,390);else{moving=false;renderCommandBuilder()}};setTimeout(step,160)}
  ...
  function injectCommandBuilderStyles(){
    ...
    style.textContent='.ppr-command-builder{display:none;margin-top:12px;padding:12px;border:1px solid rgba(125,211,252,.24);border-radius:18px;background:rgba(15,23,42,.76);gap:10px}.ppr-command-builder h2{margin:0;color:#f8fafc;font-size:16px}.ppr-command-builder p{margin:0;color:#b6c7d9}.ppr-command-builder-preview{padding:10px;border-radius:12px;background:#020617;border:1px solid rgba(148,163,184,.26);color:#67e8f9;font-family:Consolas,monospace;overflow:auto}.ppr-command-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:8px}.ppr-command-option{min-height:68px;border-radius:14px;border:1px solid rgba(125,211,252,.28);background:#0f172a;color:#dbeafe;text-align:left;padding:10px;display:grid;gap:3px;cursor:pointer;touch-action:manipulation}.ppr-command-option strong{font-size:13px;color:#f8fafc}.ppr-command-option code{color:#67e8f9;font-family:Consolas,monospace;font-weight:900}.ppr-command-option span{color:#94a3b8;font-size:11px}.ppr-command-option.primary{border-color:rgba(34,197,94,.55);background:linear-gradient(180deg,rgba(22,101,52,.92),rgba(15,23,42,.95));box-shadow:0 0 18px rgba(34,197,94,.12)}.ppr-command-note{font-size:12px;color:#93c5fd}@media(max-width:760px){#terminal{display:none!important}.ppr-command-builder{display:grid;position:relative;z-index:10;box-shadow:none}.ppr-command-grid{grid-template-columns:1fr}.ppr-command-option{min-height:64px}.console{padding-bottom:10px}}@media(min-width:761px){.ppr-command-builder{display:none!important}}';
    document.head.appendChild(style);
  }
  function addCommandBuilder(){if(q('#pprCommandBuilder'))return;injectCommandBuilderStyles();const form=q('#terminal'),consoleCard=form&&form.closest('.console');if(!form||!consoleCard)return;const panel=document.createElement('section');panel.id='pprCommandBuilder';panel.className='ppr-command-builder';panel.setAttribute('aria-label','Mobile command builder');panel.innerHTML='<div><h2>Build the next command</h2><p>Tap a command card to run it.</p></div><div id="pprCommandPreview" class="ppr-command-builder-preview">C:\\Lab&gt; choose a command</div><div id="pprCommandOptions" class="ppr-command-grid"></div><p class="ppr-command-note">Learn the pattern: check your IP, then ping the next gateway or target.</p>';consoleCard.appendChild(panel)}
  function renderCommandBuilder(){const holder=q('#pprCommandOptions');if(!holder)return;holder.innerHTML='';commandOptions().forEach(option=>{const button=document.createElement('button');button.type='button';button.className='ppr-command-option '+(option.kind||'support');button.innerHTML='<strong>'+option.label+'</strong><code>C:\\Lab&gt; '+option.command+'</code><span>'+option.meta+'</span>';button.addEventListener('click',()=>runBuiltCommand(option.command));holder.appendChild(button)})}
  function runBuiltCommand(command){const input=q('#cmd'),preview=q('#pprCommandPreview');if(input)input.value=command;if(preview)preview.textContent='C:\\Lab> '+command;if(handleCommand(command)){if(input)input.value='';setTimeout(()=>{classify();renderCommandBuilder()},140);setTimeout(renderCommandBuilder,700)}}
  function handleCommand(raw){const cmd=String(raw||'').trim().toLowerCase().replace(/\s+/g,' ');if(!cmd)return false;if(cmd==='ipconfig'){setLog(ipconfigText(current));return true}if(cmd==='ping 10.10.5.1'){unlockZone('lan');setLog(pingReply('10.10.5.1','ACCESS-SW.office.local','LAN access layer','ping 10.10.5.254 to test the router/gateway.'));animateTo('ACCESS-SW');return true}if(cmd==='ping 10.10.5.254'){if(!unlocked.has('lan')){setLog('C:\\Lab> '+raw+'\n\nRequest timed out.\nPatch: You have not mapped the local LAN yet. Try ping 10.10.5.1 first.');return true}unlockZone('gateway');setLog(pingReply('10.10.5.254','ROUTER-A.office.local','gateway/router layer','ping 10.20.0.1 to test the core switch.'));animateTo('ROUTER-A');return true}if(cmd==='ping 10.20.0.1'){if(!unlocked.has('gateway')){setLog('C:\\Lab> '+raw+'\n\nDestination host unreachable.\nPatch: Find the router/gateway first: ping 10.10.5.254.');return true}unlockZone('core');setLog(pingReply('10.20.0.1','CORE-SW.office.local','core office network','ping 10.20.7.1 to test the secure server gateway.'));animateTo('CORE-SW');return true}if(cmd==='ping 10.20.7.1'){if(!unlocked.has('core')){setLog('C:\\Lab> '+raw+'\n\nRequest timed out.\nPatch: The secure server network is hidden behind the core. Try ping 10.20.0.1 first.');return true}unlockZone('secure');setLog(pingReply('10.20.7.1','VAULT-GATE.office.local','secure server gateway','ping 10.20.7.15 or ping fileserver to confirm the target.'));animateTo('VAULT-GATE');return true}if(cmd==='ping 10.20.7.15'||cmd==='ping fileserver'){if(!unlocked.has('secure')){setLog('C:\\Lab> '+raw+'\n\nPing request could not reach the target network.\nPatch: FILESERV is behind the secure gateway. Try ping 10.20.7.1 first.');return true}unlockZone('target');setLog(pingReply('10.20.7.15','FILESERV.office.local','FILESERV target','Golden Egg route confirmed.'));animateTo('FILESERV');setTimeout(()=>{const r=q('#reward');if(r)r.hidden=false},2600);return true}return false}
