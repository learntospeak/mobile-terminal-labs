(function () {
  ...
  scenario.steps = [
    ...
    { objective: "Verify the copied note opens from the Handover folder.", commandFamily: "type", context: "A technician verifies the destination copy, not just the original file.", hints: ["Move into Handover or read the file with its full path.", "The copied file should still be summary.txt.", "Try `type C:\\Lab\\Reports\\Handover\\summary.txt`."], demoCommand: "type C:\\Lab\\Reports\\Handover\\summary.txt", accepts: [rawMatch(/^type\s+summary\.txt$/i), rawMatch(/^type\s+C:\\Lab\\Reports\\Handover\\summary\.txt$/i), rawMatch(/^type\s+C:\/Lab\/Reports\/Handover\/summary\.txt$/i)], partials: [{ match: { finalCwd: "C:/Lab/Reports/Handover" }, feedback: "Good. You are in Handover now. Use type summary.txt to verify the copied file." }], exploration: commonExploration }
  ];
